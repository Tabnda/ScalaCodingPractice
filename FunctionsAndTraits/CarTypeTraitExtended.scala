...
object CarTypeTraitExtended extends App{
  /***
   Write a Scala program and use an abstract class to define a Vehicle type with a name property.
  Next, create two case classes, named, Car, and Bike, to model a given Vehicle - using, of course,
  the Vehicle type. Create a trait named VehicleInventory that has a type parameter, V,
  and will define the methods only - that is, methods without implementations - to model a typical
  create(), read(), update(), and delete(), CRUD operations. Each of these CRUD methods will have
  an input parameter for a given vehicle, where the type is based from the trait's type parameter.
  Using the VehicleInventory trait, create a class named VehicleInventorySystem that implements
  all
  the CRUD methods of the base VehicleInventory trait. Next, create objects, or instances,
  for the
  following cars, and bikes:
- bmw car 5 series
- mazda car 3 series
...
- bmw bike r 2000
  ...
  the honda bike to the update() method, and the bmw bike to the delete method.
  The output for your Scala program should be as follows:
  Output:
